...
import nn
from nn import Dropout, EmbeddingLayer, RecurrentLayer, Layer, RCNN, ScalarRCNN, apply_dropout
from nn import get_activation_by_name, create_optimization_updates
from nn.evaluation import evaluate_average
import nn.initialization as nn_init
...
class SimpleRNN(object):
    ...
    def ready(self, args, train):
        ...
        # set the init param scale for SGD
        if args["rcnn"]:
            nn_init.default_init_scaling_factor = 1
        else:
            nn_init.default_init_scaling_factor = 1
        ...
        l2_cost = 0
        if args["l2_reg"]>0:
            l2_reg = args["l2_reg"]*0.5
            for p in self.params:
                l2_cost += T.sum(p**2)*l2_reg
    ...
    def train(self, args, train, dev, test=None):
        ...
        cost = T.sum(self.nll) / self.idxs.shape[1] + self.l2_cost
        ...
        decay_lr = args["decay_lr"] and args["learning"].lower() != "adadelta" and \
                args["learning"].lower() != "adagrad" and args["learning"].lower() != "sgd"
        ...
        for epoch in xrange(max_epoch):
            ...
            if args["decay_lr"] and epoch >= args["lr_decay_epoch"]:
                lr.set_value(np.float32(lr.get_value()*args["lr_decay"]))

            start_time = time.time()

            prev_state = [ np.zeros((batch_size, self.n_d*(self.order+1)),
    ...
    def evaluate(self, eval_func, dev, batch_size, unroll_size):
        ...
        predictions = [ ]
        init_state = [ np.zeros((batch_size, self.n_d*(self.order+1)),
                        dtype=theano.config.floatX) for i in xrange(depth) ]
